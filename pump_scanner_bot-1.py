...
async def status_cmd(message: types.Message):
    await message.answer(
        "📊 <b>Статус</b>\n"
        f"📈 Потолок роста в окне: {settings['percent']}% за {settings['window_min']} мин (алерт если рост ещё не превысил)\n"
        ...
        try:
            ...
            for item in data:
                ...
                if should_alert:
                    ...
                    # Считаем объёмные метрики ВСЕГДА (для информации в алерте),
                    # а фильтруем по ним (should_alert = False) только если
                    # соответствующий порог реально включён (> 0).
                    vh_pct = vm_pct = None
                    if should_alert:
