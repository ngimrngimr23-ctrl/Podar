...
settings = {
    ...
    # ВАЖНО: если vh_percent == 0 И vm_percent == 0 — алерты не будут срабатывать
    # вообще (нет первичного триггера).
...
hour_baseline_cache = {}     # symbol -> {"ts": fetched_at, "ref": float (макс. за период), "cur": float}
day_baseline_cache = {}      # symbol -> {"ts": fetched_at, "ref": float (макс. за период), "cur": float}
...
async def start_cmd(message: types.Message):
    ...
    await message.answer(
        ...
        "Логика: <b>/vh и /vm — первичный критерий</b> (алерт вообще возможен, "
        ...
        "🔊 <b>ОБЪЁМ (первично)</b>\n"
        f"/vh 5 — мин. % превышения нового максимума часа над предыдущим рекордом за ~7 дней (0=выкл)\n"
        f"   └ сейчас: <b>{fmt_pct('vh_percent')}</b>\n"
        f"/vm 5 — мин. % превышения нового максимума минуты над предыдущим рекордом за сутки (0=выкл)\n"
...
async def set_vh(message: types.Message, command: CommandObject):
    try:
        ...
        await message.answer(
            ...
            f"<i>Напоминание: алерт возможен, только если включён /vh и/или /vm.</i>",
            parse_mode="HTML")
    except Exception:
        await message.answer("❌ Ошибка. Пример: /vh 5 (0 = выключить)")
...
async def set_vm(message: types.Message, command: CommandObject):
    try:
        ...
        await message.answer("❌ Ошибка. Пример: /vm 5 (0 = выключить)")
...
async def debug_cmd(message: types.Message):
    ...
    lines = [
        ...
        f"2️⃣ Прошли мин. объём 24ч (/v): {debug_stats['passed_volume_floor']}",
        f"3️⃣ Прошли кулдаун: {debug_stats['passed_cooldown']}",
        f"4️⃣ Прошли фильтр объёма (/vh /vm): {debug_stats['passed_volume_filter']}",
    ...
    lines.append("")
    lines.append(
        "💡 Если шаг 2→3 или 3→4 обнуляется почти полностью — скорее всего API "
        "klines рвётся по таймауту/рейт-лимиту (смотри ошибку выше). Если шаг "
        "4→5 обнуляется — вероятно, ещё не накоплено полное окно истории цен "
...
async def status_cmd(message: types.Message):
    volume_active = settings["vh_percent"] > 0 or settings["vm_percent"] > 0
    await message.answer(
        "📊 <b>Статус</b>\n"
        f"🔊 Новый макс. объём/час (запас над рекордом за 7д): {fmt_pct('vh_percent')}\n"
        f"🔊 Новый макс. объём/мин (запас над рекордом за сутки): {fmt_pct('vm_percent')}\n"
        f"💰 Мин. объём 24ч: {settings['min_volume']:,}$\n"
        f"{'✅ Первичный триггер активен' if volume_active else '⚠️ Первичный триггер ВЫКЛЮЧЕН — алертов не будет, включи /vh или /vm'}\n\n"
...
async def get_hour_volume_anomaly(symbol):
    ...
    объёма за период или нет (а не превышение над средним). Кэшируем на
    BASELINE_TTL секунд на пару, чтобы не заваливать API klines-запросами.
    """
    cached = hour_baseline_cache.get(symbol)
    now = time.time()
    if cached and (now - cached["ts"]) < BASELINE_TTL:
        return cached["cur"], cached["ref"]
    ...
    hour_baseline_cache[symbol] = {"ts": now, "ref": prior_max, "cur": last_vol}
    return last_vol, prior_max


async def get_minute_volume_anomaly(symbol):
    """
    Средний... нет, теперь МАКСИМАЛЬНЫЙ объём минутных свечей за последние ~сутки
    (сколько отдаст API за один запрос, обычно до 1000 баров ≈ 16.6ч). Сравниваем
    последнюю закрытую минуту с этим максимумом — т.е. это новый рекорд объёма
    за период или нет.
    """
    ...
    try:
        ...
        prior_vols = [float(bar[7]) if len(bar) > 7 else float(bar[5]) for bar in closed[:-1]]
    except Exception:
        return 0.0, 0.0

    prior_max = max(prior_vols) if prior_vols else 0.0
    day_baseline_cache[symbol] = {"ts": now, "ref": prior_max, "cur": last_vol}
    return last_vol, prior_max
...
# значение (см. hour_baseline_cache / day_baseline_cache), т.е. цифра может быть
# "не идеально свежей" (до BASELINE_TTL секунд), это сознательный компромисс
# между полнотой охвата и лимитами API.
REFRESH_BATCH = 20
...
def get_cached_pct(symbol):
    """Достаёт последние известные % превышения объёма из кэша, без нового запроса."""
    vh_pct = vm_pct = None
...
async def parser_task():
    ...
    while True:
        ...
        try:
            ...
            if settings["vh_percent"] > 0 or settings["vm_percent"] > 0:
            ...
            for item in data:
                ...
                # ============ ШАГ: КУЛДАУН ============
                # Дешёвая проверка раньше объёма/цены — если монета недавно уже
                # алертилась, дальше по ней вообще нет смысла считать.
                ...
                # Хотя бы один из /vh, /vm должен быть включён (>0) и выполнен —
                # иначе монета не проходит дальше вообще. Если оба выключены (0),
                # алертов не будет: боту нужен хотя бы один объёмный триггер.
                vh_pct, vm_pct = get_cached_pct(pair)
                volume_ok = False
                ...
                if should_alert:
                    ...
                    if vh_pct is not None:
                        lines.append(f"🔊 Новый рекорд объёма/час (vs 7д): <b>{vh_pct:+.1f}%</b>")
                    if vm_pct is not None:
                        lines.append(f"🔊 Новый рекорд объёма/мин (vs сутки): <b>{vm_pct:+.1f}%</b>")
