...
async def set_vh(message: types.Message, command: CommandObject):
    try:
        ...
        await message.answer(
            f"✅ ПЕРВИЧНЫЙ критерий — превышение объёма часа над средним по этому часу за 7д: <b>{fmt_pct('vh_percent')}</b>\n"
...
async def set_vm(message: types.Message, command: CommandObject):
    try:
        ...
        await message.answer(
            f"✅ ПЕРВИЧНЫЙ критерий — превышение объёма минуты над средним за сутки: <b>{fmt_pct('vm_percent')}</b>\n"
            f"<i>Напоминание: алерт возможен, только если включён /vh и/или /vm.</i>",
            parse_mode="HTML")
    except Exception:
...
async def get_hour_volume_anomaly(symbol):
    ...
    # каждый запрос без исключения (это было причиной, почему /vh никогда не
    # срабатывал: бейзлайн просто никогда не считался).
    if len(data) < 30:
        return 0.0, 0.0
...
def get_cached_pct(symbol):
    ...
    if h:
        vh_pct = pct_over_baseline(h["cur"], h["avg"])
    m = day_baseline_cache.get(symbol)
    if m:
        vm_pct = pct_over_baseline(m["cur"], m["avg"])
