...
settings = {
    # --- Цена ---
    "percent": 3.0,          # ПОТОЛОК роста цены в окне: алерт только если рост
                             # ЕЩЁ НЕ превысил это значение (ловим начало движения)
    ...
    # % превышения объёма последнего закрытого ДНЯ над средним дневным объёмом
    # за последние до 60 дней (для молодых монет — сколько дней реально есть).
    "vd_percent": 0.0,       # 0 = выкл
...
day_baseline_cache = {}      # symbol -> {"ts": fetched_at, "avg": float, "cur": float} (для vm: минутный объём vs сутки)
daily_vol_baseline_cache = {}  # symbol -> {"ts": fetched_at, "avg": float, "cur": float} (для vd: дневной объём vs 60д)

# Диагностика: что происходило на последнем проходе сканера — сколько монет
# прошло каждый этап фильтрации, и последняя ошибка klines-запроса (если были).
scan_stats = {
    "total_pairs": 0,
    "passed_min_volume": 0,
    "passed_cooldown": 0,
...
async def start_cmd(message: types.Message):
    ...
    await message.answer(
        ...
        "❗️ <b>Объём — основной критерий отбора</b>: если включён хотя бы один из /vh, /vm, /vd, "
        "монета сначала проверяется по нему, и только потом — по цене (доп. фильтр).\n\n"

        "⚙️ <b>ЦЕНА (дополнительно)</b>\n"
        f"/p 5 — ПОТОЛОК роста в окне: алерт только если рост ещё НЕ превысил это значение (ловим начало движения)\n"
        ...
        "⚙️ <b>ОБЪЁМ (основной критерий)</b>\n"
        f"/vh 200 — мин. % превышения объёма текущего часа над средним по этому же часу суток (0=выкл)\n"
        ...
        f"/vd 150 — мин. % превышения объёма последнего дня над средним дневным за последние (до) 60 дней (0=выкл)\n"
        f"   └ сейчас: <b>{fmt_pct('vd_percent')}</b>\n"
        f"/v 200000 — мин. объём торгов за 24ч в $, ниже которого пара игнорируется совсем\n"
        ...
        f"/debug — воронка последнего прохода: сколько монет прошло каждый фильтр (для диагностики, если алертов нет)"
        , parse_mode="HTML")
...
async def set_percent(message: types.Message, command: CommandObject):
    try:
        ...
        await message.answer(
            f"✅ Потолок роста в окне: алерт пройдёт, только если рост ещё НЕ превысил <b>{settings['percent']}%</b>",
            parse_mode="HTML")
    except Exception:
...
async def set_vd(message: types.Message, command: CommandObject):
    try:
        settings["chat_id"] = message.chat.id
        val = abs(_parse_float(command.args))
        settings["vd_percent"] = val
        await message.answer(
            f"✅ Мин. превышение объёма дня над средним за последние (до) 60 дней: <b>{fmt_pct('vd_percent')}</b>",
            parse_mode="HTML")
    except Exception:
        await message.answer("❌ Ошибка. Пример: /vd 150 (0 = выключить)")
...
async def debug_cmd(message: types.Message):
    ls = scan_stats["last_run"]
    ago = f"{int(time.time() - ls)} сек назад" if ls else "ещё не запускался"
    await message.answer(
        "🔍 <b>Воронка последнего прохода сканера</b>\n"
        f"⏱ Прошёл: {ago}\n"
        f"📡 Тикер MEXC (список всех пар): {'✅ ОК' if scan_stats['ticker_ok'] else '❌ ОШИБКА — смотри логи Render'}\n"
        f"1️⃣ Всего USDT-пар от биржи: <b>{scan_stats['total_pairs']}</b>\n"
        f"2️⃣ Прошли мин. объём 24ч (/v): <b>{scan_stats['passed_min_volume']}</b>\n"
        f"3️⃣ Прошли кулдаун: <b>{scan_stats['passed_cooldown']}</b>\n"
        f"4️⃣ Прошли фильтр объёма (/vh /vm /vd): <b>{scan_stats['passed_volume_filter']}</b>\n"
        f"5️⃣ Прошли фильтр цены (/p /d /dmax): <b>{scan_stats['passed_price_filter']}</b>\n"
        f"📨 Алертов отправлено за этот проход: <b>{scan_stats['alerts_sent']}</b>\n\n"
        f"⚠️ Ошибок запросов klines за проход: <b>{scan_stats['klines_errors']}</b>\n"
        f"Последняя ошибка: <code>{scan_stats['last_klines_error'] or '—'}</code>\n\n"
        "💡 Если шаг 3→4 обнуляется почти полностью — скорее всего API klines "
        "рвётся по таймауту/рейт-лимиту (смотри ошибку выше). Если шаг 4→5 "
        "обнуляется — вероятно, бот ещё не накопил полное окно истории цен "
        f"({settings['window_min']} мин с момента запуска) — подожди и глянь /debug снова."
        , parse_mode="HTML")
...
async def status_cmd(message: types.Message):
    await message.answer(
        "📊 <b>Статус</b>\n"
        f"🔊 <b>Объём (основной критерий):</b>\n"
        f"   час vs тот же час суток: {fmt_pct('vh_percent')}\n"
        f"   минута vs сутки: {fmt_pct('vm_percent')}\n"
        f"   день vs 60 дней: {fmt_pct('vd_percent')}\n"
        f"📈 <b>Цена (доп. фильтр):</b>\n"
        f"   потолок роста в окне: {settings['percent']}% за {settings['window_min']} мин\n"
...
async def fetch_prices():
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; PumpScannerBot/1.0)"}
    last_err = None
    for attempt in range(2):  # 1 повтор, если первая попытка отвалилась
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=15)) as response:
                    if response.status == 200:
                        scan_stats["ticker_ok"] = True
                        return await response.json()
                    else:
                        body = await response.text()
                        last_err = f"HTTP {response.status}: {body[:200]}"
        except asyncio.TimeoutError:
            last_err = "TimeoutError (запрос не ответил за 15 сек)"
        except Exception as e:
            last_err = f"{type(e).__name__}: {e or 'без сообщения'}"
    if last_err:
        print(f"Ошибка API (ticker): {last_err}", flush=True)
        scan_stats["ticker_ok"] = False
        return []
    return []
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=8)) as resp:
                if resp.status == 200:
                    ...
                    scan_stats["klines_errors"] += 1
                    if scan_stats["last_klines_error"] is None:
                        scan_stats["last_klines_error"] = f"{symbol} {interval}: HTTP {resp.status}"
    except asyncio.TimeoutError:
        scan_stats["klines_errors"] += 1
        if scan_stats["last_klines_error"] is None:
            scan_stats["last_klines_error"] = f"{symbol} {interval}: TimeoutError"
    except Exception as e:
        scan_stats["klines_errors"] += 1
        if scan_stats["last_klines_error"] is None:
            scan_stats["last_klines_error"] = f"{symbol} {interval}: {type(e).__name__}: {e or 'без сообщения'}"
    return []
...
async def get_hour_volume_anomaly(symbol):
    ...
    # Раньше требовался жёсткий минимум в 30 баров — из-за этого молодые монеты
    # всегда получали (0.0, 0.0) и не проходили фильтр. Теперь берём столько,
    # сколько реально есть: нужно минимум 2 бара (1 закрытый для сравнения + текущий
    # формирующийся, который мы всё равно отбрасываем).
    if len(data) < 2:
        return 0.0, 0.0
    ...
    for bar in closed[:-1]:
        try:
            ...
            bar_vol = float(bar[7]) if len(bar) > 7 else float(bar[5])
            all_vols.append(bar_vol)
            if bar_hour == last_hour:
                same_hour_vols.append(bar_vol)
        except Exception:
            continue

    if same_hour_vols:
        # Нормальный случай: есть история именно по этому часу суток.
        avg = sum(same_hour_vols) / len(same_hour_vols)
    else:
        # Монета слишком молодая, совпадений по часу суток ещё не набралось —
        # честно берём среднее по всем доступным закрытым часовым свечам,
        # сколько бы их ни было (хоть одна), вместо занижения до 0.
        avg = sum(all_vols) / len(all_vols) if all_vols else 0.0
...
async def get_minute_volume_anomaly(symbol):
    ...
    # Аналогично часовой функции: берём сколько есть, минимум 2 бара.
    if len(data) < 2:
        return 0.0, 0.0
...
async def get_day_volume_anomaly(symbol):
    """
    /vd: берём последнюю ЗАКРЫТУЮ суточную свечу и сравниваем со средним дневным
    объёмом за предыдущие дни. Целимся в 60 дней истории, но если монета моложе —
    честно используем столько дней, сколько реально есть (хоть 1), вместо 0.
    """
    cached = daily_vol_baseline_cache.get(symbol)
    now = time.time()
    if cached and (now - cached["ts"]) < BASELINE_TTL:
        return cached["cur"], cached["avg"]

    data = await fetch_klines(symbol, "1d", 61)  # до 60 дней базы + текущий формирующийся день
    if len(data) < 2:
        return 0.0, 0.0

    closed = data[:-1]  # последняя свеча — текущий незакрытый день, отбрасываем
    try:
        last_vol = float(closed[-1][7]) if len(closed[-1]) > 7 else float(closed[-1][5])
        vols = [float(bar[7]) if len(bar) > 7 else float(bar[5]) for bar in closed[:-1]]
    except Exception:
    ...
    avg = sum(vols) / len(vols) if vols else 0.0
    daily_vol_baseline_cache[symbol] = {"ts": now, "avg": avg, "cur": last_vol}
    return last_vol, avg
...
async def parser_task():
    ...
    while True:
        try:
            ...
            scan_stats["total_pairs"] = 0
            scan_stats["passed_min_volume"] = 0
            scan_stats["passed_cooldown"] = 0
            scan_stats["passed_volume_filter"] = 0
            scan_stats["passed_price_filter"] = 0
            scan_stats["alerts_sent"] = 0
            scan_stats["klines_errors"] = 0
            scan_stats["last_klines_error"] = None
            scan_stats["last_run"] = now

            for item in data:
                pair = item['symbol']
                if not pair.endswith("USDT") or pair in blacklist:
                    continue
                scan_stats["total_pairs"] += 1

                try:
                    vol = float(item['quoteVolume'])
                    if vol < settings["min_volume"]:
                        continue
                    scan_stats["passed_min_volume"] += 1
                ...
                # Кулдаун проверяем сразу — если недавно уже алертили эту монету,
                # нет смысла тратить запросы к API klines на неё.
                should_alert = True
                ...
                if not should_alert:
                    ...
                    continue
                scan_stats["passed_cooldown"] += 1

                # ===== 1) ОБЪЁМ — ОСНОВНОЙ КРИТЕРИЙ =====
                # Считаем метрики, которые реально включены (порог > 0). Если ни один
                # объёмный порог не включён, этот блок просто ничего не отфильтрует
                # (все vh/vm/vd = None), и монета пойдёт дальше на ценовые фильтры —
                # как и раньше, для обратной совместимости.
                vh_pct = vm_pct = vd_pct = None
                ...
                if should_alert and settings["vd_percent"] > 0:
                    cur_d, avg_d = await get_day_volume_anomaly(pair)
                    vd_pct = pct_over_baseline(cur_d, avg_d)
                    if vd_pct < settings["vd_percent"]:
                        should_alert = False

                if not should_alert:
                    history.append((now, price))
                    continue
                scan_stats["passed_volume_filter"] += 1

                # ===== 2) ЦЕНА — ДОПОЛНИТЕЛЬНЫЙ ФИЛЬТР =====
                # /p теперь ПОТОЛОК: пропускаем монету, только если рост в окне ЕЩЁ
                # НЕ превысил порог (т.е. движение только начинается, не разогналось).
                # ВАЖНО: day_min_rise=0 означает "выключено" — раньше код всё равно
                # требовал ch_24 >= 0, что молча резало все монеты с отрицательным
                # 24ч изменением, даже если ты явно не ставил этот фильтр. Исправлено.
                base_price_ok = have_full_window and rise <= settings["percent"]
                ...
                if not base_price_ok:
                    history.append((now, price))
                    continue
                scan_stats["passed_price_filter"] += 1
                ...
                if should_alert:
                    ...
                    if vh_pct is not None:
                        lines.append(f"🔊 Объём/час vs тот же час суток: <b>{vh_pct:+.1f}%</b>")
                    if vm_pct is not None:
                        lines.append(f"🔊 Объём/мин vs сутки: <b>{vm_pct:+.1f}%</b>")
                    if vd_pct is not None:
                        lines.append(f"🔊 Объём/день vs 60д: <b>{vd_pct:+.1f}%</b>")
                    lines.append(f"📈 В окне ({settings['window_min']}м): <b>+{rise:.2f}%</b>")
                    lines.append(f"📊 За 24 часа: <b>{ch_24:.2f}%</b>")
                    lines.append(f"📆 За 7 дней: <b>{ch_7:.2f}%</b>")
                    lines.append(f"🗓 За 30 дней: <b>{ch_30:.2f}%</b>")
