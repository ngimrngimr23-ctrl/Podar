        ...
        try:
            ...
            for item in data:
                ...
                if should_alert:
                    ...
                    if vh:
                        vol_lines.append(
                            f"🔊 <b>Час</b> (vs рекорд {fmt_span(vh['span_hours'])}): "
                            f"{fmt_money(vh['cur'])}$ <i>(было макс. {fmt_money(vh['ref'])}$, {vh['pct']:+.1f}%)</i>"
                            + (f" · колеб. цены {vh['rng']:.2f}%" if vh['rng'] is not None else "")
                        )
                    if vm:
                        vol_lines.append(
                            f"🔊 <b>{settings['vm_interval_min']}м</b> (vs рекорд {fmt_span(vm['span_hours'])}): "
                            f"{fmt_money(vm['cur'])}$ <i>(было макс. {fmt_money(vm['ref'])}$, {vm['pct']:+.1f}%)</i>"
                            + (f" · колеб. цены {vm['rng']:.2f}%" if vm['rng'] is not None else "")
                        )
                    if v4h:
                        vol_lines.append(
                            f"🔊 <b>4ч</b> (vs рекорд {fmt_span(v4h['span_hours'])}): "
                            f"{fmt_money(v4h['cur'])}$ <i>(было макс. {fmt_money(v4h['ref'])}$, {v4h['pct']:+.1f}%)</i>"
                            + (f" · колеб. цены {v4h['rng']:.2f}%" if v4h['rng'] is not None else "")
                        )
