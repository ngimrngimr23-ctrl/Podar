...
async def debug_cmd(message: types.Message):
    ...
    lines = [
        ...
        f"4️⃣ Прошли фильтр объёма (/vh /vm /v4h): {debug_stats['passed_volume_filter']}",
...
def get_cached_pct(symbol):
    """Достаёт последние известные % превышения объёма и % колебания цены из кэша, без нового запроса."""
    ...
    if f:
        ...
        v4h_rng = f.get("rng")
    return vh_pct, vm_pct, v4h_pct, vh_rng, vm_rng, v4h_rng
    ...
    while True:
        ...
        try:
            ...
            for item in data:
                ...
                vh_pct, vm_pct, v4h_pct, vh_rng, vm_rng, v4h_rng = get_cached_pct(pair)
                volume_ok = False
